function dark() {
    ...
    function f() {
      n = window.innerWidth, e = window.innerHeight, i = 1000; // 将流星数量固定为1000
      s.setAttribute("width", n), s.setAttribute("height", e);
    }
    function u() {
      h.clearRect(0, 0, n, e);
      for (var t = c.length, j = 0; j < t; j++) {
        var meteor = c[j];
        meteor.move(), meteor.fadeIn(), meteor.fadeOut(), meteor.draw();
      }
    }
    function y() {
      this.reset = function() {
        this.giant = m(3), 
        this.comet = !this.giant && !o && m(10), // 增加流星
        this.x = l(0, n - 10), 
        this.y = l(0, e), 
        this.r = l(1.1, 2.6), 
    ...
    function t() {
        // 检查页面的 data-theme 属性是否为 'dark'
        if (document.getElementsByTagName('html')[0].getAttribute('data-theme') == 'dark') {
          u(); // 如果是暗色主题，调用 u() 函数更新动画
        }
        // 递归调用 requestAnimationFrame 来维持动画循环
